 ...
 * 描述：
 */
public class UserAndDycUploadRequest extends ResultPostExecute<String> {
	public void request(final String jsonData, int flag) {
		try {
			StringRequest request = new StringRequest(Request.Method.POST,
					flag == 0 ? Config.UPLOAD_USER_DYC : Config.UPLOAD_CAR_USER_DYC,
	...
	private void parserData(String result) {
		try {
			...
			String tips = obj.get("msg").getAsString();
			if (code == 0) {//请求成功
				onPostExecute("");
			} else {
				onErrorExecute(tips);
